...
def parse_llm_json(text: str, required_keys: list[str] = None, debug: bool = False) -> dict:
    ...
    Tries:
      1. fenced JSON
      2. balanced braces
      3. repaired JSON (using json_repair library)
    """
    extractors = [
    ...
    for name, extractor in extractors:
        ...
        if raw_json:
            try:
                logger.debug(f"Attempting {name} extraction...")
                return _parse_and_validate(raw_json, required_keys)
            except json.JSONDecodeError:
                logger.debug(f"JSON decode failed for {name} strategy.")
                continue
            except JsonParsingError:
                raise  # Required key missing - bubble up immediately

    # Final attempt: repair
    raw_json = extract_json_block_balanced(text)
    if raw_json:
        try:
            logger.debug("Attempting auto-repair with json_repair...")
