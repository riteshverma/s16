...
class AgentLoop4:
        ...
        while not context.all_done():
            ...
            if not ready_steps:
                ...
                if not running_or_waiting:
                    # If no ready steps and nothing running, check if we're truly done
                    terminal_states = {'completed', 'failed', 'skipped', 'cost_exceeded', 'stopped'}
                    is_complete = all(
                        context.plan_graph.nodes[n].get('status', 'pending') in terminal_states
                        for n in context.plan_graph.nodes
                        if n != "ROOT"
                    )
                    if is_complete:
            ...
            for step_id, result in zip(ready_steps, results):
                ...
                if isinstance(result, Exception):
                    if retry_count < MAX_STEP_RETRIES:
                        ...
                        skipped_count = context.skip_dependents(step_id, reason=f"Parent {step_id} failed")
                        if skipped_count > 0:
                            log_step(f"⏭️ Skipped {skipped_count} downstream steps due to {step_id} failure", symbol="⏭️")
                        log_error(f"❌ Failed {step_id} after {MAX_STEP_RETRIES} retries: {str(result)}")
                elif result["success"]:
                    # Extract metadata (cost/tokens) from the new envelope
                    metadata = result.get("metadata", {})
                    visualizer.mark_completed(step_id)
                    await context.mark_done(
                    ...
                    if retry_count < MAX_STEP_RETRIES:
                        ...
                        # Propagate failure to downstream dependents
                        skipped_count = context.skip_dependents(step_id, reason=f"Parent {step_id} failed: {result['error']}")
                        if skipped_count > 0:
                            log_step(f"⏭️ Skipped {skipped_count} downstream steps due to {step_id} failure", symbol="⏭️")
    ...
    async def _handle_failures(self, context):
        ...
        Gathers all failed steps, summarizes the errors, and asks PlannerAgent
        to produce a recovery plan that works around the failures.
        """
        failed_steps = []
        for node_id, node_data in context.plan_graph.nodes(data=True):
        ...
        log_step(f"♻️ Attempting recovery plan for {len(failed_steps)} failed steps", symbol="🔄")
        ...
        try:
            plan_result = await self.agent_runner.run_agent(
                ...
                {
                    "original_query": context.plan_graph.graph['original_query'],
                    "planning_strategy": "recovery",
                    "globals_schema": context.plan_graph.graph.get("globals_schema", {}),
                    "file_manifest": context.plan_graph.graph.get("file_manifest", []),
                    "failed_steps": failed_steps,
                    "completed_steps": [
                        node_id for node_id, data in context.plan_graph.nodes(data=True)
                        if data.get("status") == "completed" and node_id != "ROOT"
                    ],
                }
            )
            
            if plan_result["success"] and "plan_graph" in plan_result.get("output", {}):
                new_plan = plan_result["output"]["plan_graph"]
                self._merge_plan_into_context(new_plan)
                log_step(f"✅ Recovery plan merged with {len(new_plan.get('nodes', []))} new steps", symbol="🔄")
            else:
                logger.warning("Recovery planning did not produce a valid plan")
        except Exception as e:
            logger.error(f"Recovery planning failed: {e}", exc_info=True)
            log_error(f"Recovery planning failed: {e}")
