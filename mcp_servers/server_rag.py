...
"""
    ...
    try:
        ...
        # Capture Ollama error body on non-2xx (e.g. 400 Bad Request)
        if not response.ok:
            try:
                err_body = response.json()
                ollama_msg = err_body.get("error", response.text[:500] if response.text else "Unknown")
            except Exception:
                ollama_msg = response.text[:500] if response.text else str(response.reason)
            mcp_log("ERROR", f"Ollama chat {response.status_code}: {ollama_msg}")
            hint = ""
            if response.status_code == 400:
                hint = f" Check that the model '{VISION_MODEL}' is installed (ollama pull {VISION_MODEL}) and supports chat."
            return f"Error: Could not reach the AI model for this document query. ({response.status_code} {response.reason}: {ollama_msg}).{hint}"
        ...
        mcp_log("ERROR", f"Ollama ask timeout: {e}")
        return f"Error: AI model request timed out. Try increasing ollama timeout in settings or using a smaller model."
    except requests.exceptions.ConnectionError as e:
        mcp_log("ERROR", f"Ollama connection failed: {e}")
        return f"Error: Could not connect to Ollama at {OLLAMA_CHAT_URL}. Is Ollama running?"
    except Exception as e:
...
Keep your response concise (2-3 sentences max)."""
        ...
        with requests.post(OLLAMA_URL, json={
            ...
            if not result.ok:
                try:
                    ...
                    ollama_msg = err_body.get("error", result.text[:500] if result.text else "Unknown")
                except Exception:
                    ollama_msg = result.text[:500] if result.text else str(result.reason)
                mcp_log("ERROR", f"Ollama generate {result.status_code}: {ollama_msg}")
                return f"[Image caption failed: {result.status_code} {ollama_msg}. Ensure model '{VISION_MODEL}' is installed (ollama pull {VISION_MODEL}) and supports vision.]"
        ...
        mcp_log("ERROR", "Ollama caption timeout")
        return "[Image caption timed out]"
    except requests.exceptions.ConnectionError:
        mcp_log("ERROR", "Ollama connection failed for caption")
        return f"[Could not connect to Ollama. Is it running?]"
    except Exception as e:
