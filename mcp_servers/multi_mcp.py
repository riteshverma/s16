    ...
    def _load_disabled_tools(self):
        if self.disabled_tools_path.exists():
            try:
                ...
                logger.warning(f"Failed to load disabled tools: {e}")
    ...
    async def _start_server(self, name: str, config: dict):
        ...
        try:
            ...
            if server_type == "local-script":
                ...
                # Normalize uv args in case config included an old absolute path
                # We will inject the correct --directory and --with-requirements below.
                cleaned_args = []
                skip_next = False
                for arg in args:
                    if skip_next:
                        skip_next = False
                        continue
                    if arg in ("--directory", "--with-requirements"):
                        skip_next = True
                        continue
                    if arg.startswith("--directory=") or arg.startswith("--with-requirements="):
                        continue
            ...
            async with asyncio.timeout(20): # 20s timeout (increased for installations)
                ...
                if name in self._cached_metadata:
                    ...
                    print(f"  [green]OK[/green] [cyan]{name}[/cyan] connected. Tools: {len(result.tools)}")
                
                self.sessions[name] = session

        except TimeoutError:
             logger.warning(f"MCP server '{name}' timed out during startup.")
             print(f"  [yellow]{name}[/yellow] timed out during startup.")
        except Exception as e:
            logger.error(f"MCP server '{name}' failed to start: {e}", exc_info=True)
            print(f"  [red]{name}[/red] failed to start: {e}")
        except BaseException as e:
            logger.critical(f"MCP server '{name}' critical failure: {e}", exc_info=True)
    ...
    async def start(self):
        ...
        for name, config in self.server_configs.items():
            if config.get("enabled", True):
                ...
                print(f"  [dim]Skipping disabled server: {name}[/dim]")
