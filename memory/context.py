...
class ExecutionContextManager:
    ...
    def get_ready_steps(self):
        ...
        - All predecessor nodes are in 'completed' status
        
        Steps that are already completed, failed, running, skipped, or in 
        any other terminal state are excluded.
        """
        ready = []
        # Terminal or in-progress states that should not be scheduled
        non_schedulable = {'completed', 'failed', 'running', 'skipped', 'cost_exceeded', 'stopped', 'waiting_input'}
    ...
    def get_inputs(self, reads):
        ...
        for read_key in reads:
            if read_key in globals_schema:
                ...
                logger.warning(f"Missing dependency: '{read_key}' not found in globals_schema. Available: {list(globals_schema.keys())}")
                
        return inputs

    def skip_dependents(self, failed_step_id: str, reason: str = "") -> int:
        """Mark all downstream dependents of a failed step as 'skipped'.
        
        Uses BFS to traverse all transitive successors and marks them skipped
        so they don't block completion or get executed.
        ...
        """
        skipped_count = 0
        # Get all transitive descendants using NetworkX
        try:
            descendants = nx.descendants(self.plan_graph, failed_step_id)
        except nx.NetworkXError:
            return 0
        
        for dep_id in descendants:
            dep_data = self.plan_graph.nodes[dep_id]
            current_status = dep_data.get('status', 'pending')
            # Only skip pending/waiting nodes (don't touch completed or already failed ones)
            if current_status in ('pending', 'waiting_input'):
                dep_data['status'] = 'skipped'
                dep_data['error'] = reason or f"Skipped: dependency {failed_step_id} failed"
                dep_data['end_time'] = datetime.utcnow().isoformat()
                skipped_count += 1
                logger.info(f"Skipped {dep_id} due to failure of {failed_step_id}")
    ...
    def all_done(self):
        """Check if all steps are in a terminal state (completed, failed, or skipped)."""
        terminal_states = {'completed', 'failed', 'skipped', 'cost_exceeded', 'stopped'}
        return all(
            self.plan_graph.nodes[node_id].get('status', 'pending') in terminal_states
            for node_id in self.plan_graph.nodes
        )
    ...
    def get_execution_summary(self):
        ...
        skipped = sum(1 for node_id in self.plan_graph.nodes 
                     if node_id != "ROOT" and 
                     self.plan_graph.nodes[node_id].get('status') == 'skipped')
